...
class ToolExecutor {
  ...
  static async functionDeleteFile(params, targetDir) {
    try {
      ...
      const fullPath = await this.ensurePathWithinBase(targetDir, filePath);
  ...
  static async ensurePathWithinBase(baseDir, userPath) {
    const candidate = path.resolve(baseDir, userPath);
    const [baseReal, candidateReal] = await Promise.all([fs.realpath(baseDir), fs.realpath(candidate)]);
    let baseNorm = baseReal;
    let candNorm = candidateReal;
    if (process.platform === 'win32') {
      baseNorm = baseNorm.toLowerCase();
      candNorm = candNorm.toLowerCase();
    }
    if (candNorm === baseNorm || candNorm.startsWith(baseNorm + path.sep)) {
  ...
  static async handleDeleteFile(command, workingDir) {
    ...
    try {
      ...
      const filePath = await this.ensurePathWithinBase(targetDir, fileName);
  ...
  static async handleDeleteDirectory(command, workingDir) {
    ...
    try {
      ...
      const fullPath = await this.ensurePathWithinBase(targetDir, dirPath);
